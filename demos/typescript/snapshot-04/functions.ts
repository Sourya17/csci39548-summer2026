...
function priorityLabel(p = 3) {                    // p inferred as number
  if (p === 1) return "HIGH";
  if (p === 2) return "MED";
  return "LOW";
}
...
// ------------------------------------------------------------
// 7. Function TYPE ALIASES — name the shape of a function.
//
// `type Name = (...) => ReturnType;`
//
// Useful when you pass functions around (callbacks, event handlers)
// or when the same signature appears in multiple places.
// ------------------------------------------------------------

// Alias for "a function that takes a number and returns a number".
type NumberTransformer = (n: number) => number;

const double:  NumberTransformer = (n) => n * 2;        // matches the shape
const triple:  NumberTransformer = (n) => n * 3;
...
// shape works.
function applyTo10(fn: NumberTransformer): number {
  return fn(10);
}
...
function formatTime(minutes: number, includeHours: boolean = false): string {
  if (!includeHours) return `${minutes} min`;
...
// `export {}` makes this file a module — keeps top-level names
// (like `add`, `greet`) from clashing with TS's DOM globals.
// You'll see this pattern when you start writing real modules.
export {};
